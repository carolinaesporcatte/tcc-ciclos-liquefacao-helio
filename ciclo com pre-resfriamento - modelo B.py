...
PC2 = MovingBoundaryHeatExchanger("PC2")
...
va = Valve("valvula JT")
sink1 = Sink("sumidouro líquido/He")
sink2 = Sink("sumidouro gás/He")
...
c1a = Connection(HEx1, 'out1', PC2, 'in1', label='1a')
c1b = Connection(PC2, 'out1', PC1, 'in1', label='1b')
...
cL = Connection(sep, 'out1', sink1 , 'in1', label='L')
...
c14 = Connection(HEx1, 'out2', sink2, 'in1', label='14')

ca = Connection(fonte2, 'out1', PC1, 'in2', label='N2a')
cb = Connection(PC1, 'out2', PC2, 'in2', label='N2b')
cc = Connection(PC2, 'out2', sinkN2, 'in1', label='N2c')
...
my_plant.add_conns(c0,c0a,c0b,c0c,c1,c1a,c1b,c2b,c3,c3a,c3b,c4,c5,c5a,c5b,c6,c7,c8,cL,cG,c9,c9a,c9b,c10,c11,c11a,c11b,c12,c13,c14,ca,cb,cc,ee_in, ee1, ee2, ee3, ee4,h1, h2)
...
c7.set_attr(T=7.5)
c8.set_attr(p = Pmin)
ca.set_attr(p = Pmin, fluid = {'N2':1},x=0) 
cb.set_attr(x=1)
...
fuel = {"inputs": ["ee_in","0","N2a"], "outputs": ["14","N2c"]}
...
df_pmax.to_excel("LN2B_pmax.xlsx", index=False)
...
for Pmax in Pmax_values:
    try:
        ...
        componentes_interesse = ["compressor 1","compressor 2","expansor 1","expansor 2","HEx1","HEx2","HEx3","HEx4","HEx5","HEx6","PC1","PC2","resfriador 1","resfriador 2","valvula JT"]
...
df_ex_comp.to_excel("LN2B_exergia_componentes_pmax.xlsx", index=False)
...
for eta_is_exp in eta_values:
    try:
        ...
        if not my_plant.converged:
            ...
            results_eta.append([eta_is_exp, np.nan, np.nan,np.nan,np.nan])
            continue
...
df_eta.to_excel("LN2B_eta_exp.xlsx", index=False)
...
df_ex_comp_eta.to_excel("LN2B_exergia_componentes_eta_exp.xlsx", index=False)


#Análise da variação da efetividade dos trocadores 
#loop dos parâmetros y, COP e eficiencia exergetica do ciclo
eff_values = np.arange(0.76,0.99,0.01)
results_ef =[]
for ef_HEx in eff_values:
    try:
        ...
        if not my_plant.converged:
            ...
            results_ef.append([ef_HEx, np.nan, np.nan, np.nan,np.nan])
            continue
...
df_eff.to_excel("LN2B_eff.xlsx", index=False)
...
df_ex_comp_ef.to_excel("LN2B_exergia_componentes_efetividade.xlsx", index=False)
...
df_eff.to_excel("LN2B_pr.xlsx", index=False)
...
df_ex_comp_pr.to_excel("LN2B_exergia_componentes_pr.xlsx", index=False)


#Análise da variação dos parametros x1 e x2,sendo que x2 é fixo e x1 é variado num intervalo determinado
results = []
x2_values = [0.5]
x1_values = np.arange(0.15, 0.37, 0.01)

#combinações de x1 e x2 
#x2=0.15 -> x1 0.15-0.5   
#x2=0.2  -> x1 0.15-0.44
#x2=0.25 -> x1 0.15-0.58  
#x2=0.3  -> x1 0.15-0.53
#x2=0.35 -> x1 0.15-0.49  
#x2=0.4  -> x1 0.15-0.44
#x2=0.45 -> x1 0.15-0.39 
#x2=0.5  -> x1 0.15-0.36
#x2=0.55 -> x1 0.15-0.32  
#x2=0.6  -> x1 0.15-0.28   
#x2=0.65 -> x1 0.15-0.22
...
df_x1.to_excel("LN2B_x2=0.5.xlsx", index=False)
